...
void SimpleShadowmapRender::AllocateResources() {
    ...
    shadowMap = m_context->createImage
            (etna::Image::CreateInfo
                     {
    ...
    constants = m_context->createBuffer
    ...
    soapSource = m_context->createImage
            (etna::Image::CreateInfo
                     {
                             .extent = vk::Extent3D{1024, 1024, 1},
                             .name = "soap_source",
    ...
    soapTarget = m_context->createImage
            (etna::Image::CreateInfo
                     {
                             .extent = vk::Extent3D{1024, 1024, 1},
                             .name = "soap_target",
...
void SimpleShadowmapRender::loadShaders() {
    ...
    etna::create_program("soap",
                         {VK_GRAPHICS_BASIC_ROOT"/resources/shaders/soap.comp.spv"});
}
...
void SimpleShadowmapRender::SetupSimplePipeline() {
    ...
    m_shadowPipeline = pipelineManager.createGraphicsPipeline
            ("simple_shadow",
             {
                     .vertexShaderInput = sceneVertexInputDesc,
                     .fragmentShaderOutput =
    ...
    m_soapPipeline = pipelineManager.createComputePipeline
            ("soap",
             {}
            );
}
...
void SimpleShadowmapRender::BuildCommandBufferSimple(VkCommandBuffer a_cmdBuff, VkImage a_targetImage,
    ...
    //
    {
        etna::RenderTargetState renderTargets(a_cmdBuff, {1024, 1024}, {}, shadowMap);
    ...
    //
    {
        ...
        auto set = etna::create_descriptor_set
                (simpleMaterialInfo.getDescriptorLayoutId(0), a_cmdBuff,
                 {
        ...
        etna::RenderTargetState renderTargets(a_cmdBuff,
                                              {m_width, m_height},
                                              {{soapSource.get(), soapSource.getView({})}},
                                              mainViewDepth);
    ...
    // Make the image soapy
    {
        auto soapProgram = etna::get_shader_program("soap");

        auto set = etna::create_descriptor_set
                (soapProgram.getDescriptorLayoutId(0), a_cmdBuff,
                 {
                         etna::Binding{0, soapSource.genBinding(defaultSampler.get(),
                                                                vk::ImageLayout::eGeneral)},
                         etna::Binding{1, soapTarget.genBinding(defaultSampler.get(),
                                                                vk::ImageLayout::eGeneral)}
                 });

        etna::flush_barriers(a_cmdBuff);

        vkCmdBindPipeline(a_cmdBuff, VK_PIPELINE_BIND_POINT_COMPUTE, m_soapPipeline.getVkPipeline());

        VkDescriptorSet descriptorSet = set.getVkSet();
        vkCmdBindDescriptorSets(a_cmdBuff, VK_PIPELINE_BIND_POINT_COMPUTE, m_soapPipeline.getVkPipelineLayout(), 0, 1,
    ...
    // Blit the soapy image to the screen
    {
        etna::set_state(a_cmdBuff, soapSource.get(),
        ...
        etna::set_state(a_cmdBuff, soapTarget.get(),
                        vk::PipelineStageFlagBits2::eBlit,
...

        {
            ...
            vkCmdBlitImage(a_cmdBuff,
                           soapTarget.get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
